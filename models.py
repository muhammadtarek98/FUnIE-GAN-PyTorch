...
class TotalGenLoss(torch.nn.Module):
...
def build_conv_block(in_chans: int, out_chans: int, kernel_size: int = 3, stride: int = 2, padding: int = 1,
                     use_bn: bool = True, bn_momentum: float = 0.8,
                     use_leaky: bool = False):
    layers = []
    layers.append(torch.nn.Conv2d(in_channels=in_chans, out_channels=out_chans, kernel_size=kernel_size, stride=stride,
                                  padding=padding))
    if use_leaky:
    ...
    if use_bn:
        layers.append(torch.nn.BatchNorm2d(num_features=out_chans, momentum=bn_momentum))
    return torch.nn.Sequential(*layers)


def build_deconv_block(in_chans: int, out_chans: int, use_bn: bool = True):
    layers = []
    layers.append(torch.nn.Upsample(scale_factor=2,
                                    mode="bilinear", align_corners=True))
    layers.append(torch.nn.Conv2d(in_channels=in_chans, out_channels=out_chans, kernel_size=3, stride=1, padding=1))
    layers.append(torch.nn.ReLU(inplace=True))
    if use_bn:
        layers.append(torch.nn.BatchNorm2d(num_features=out_chans, momentum=0.8))
...
class FUnIEGeneratorV1(nn.Module):
    def __init__(self, n_feats=32):
        ...
        self.conv1 = build_conv_block(
            in_chans=3, out_chans=n_feats, kernel_size=5, padding=2, use_bn=False)
        self.conv2 = build_conv_block(in_chans=n_feats, out_chans=n_feats * 4, kernel_size=4)
        self.conv3 = build_conv_block(in_chans=n_feats * 4, out_chans=n_feats * 8, kernel_size=4)
        self.conv4 = build_conv_block(in_chans=n_feats * 8, out_chans= n_feats * 8)
        self.conv5 = build_conv_block(in_chans=n_feats * 8, out_chans=n_feats * 8)
        ...
        self.final = torch.nn.Conv2d(n_feats * 2, out_channels=3, kernel_size=3, stride=1,padding= 1)
